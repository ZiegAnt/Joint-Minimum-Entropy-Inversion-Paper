...
class JointEntropyInversion(object):
    ...
    def __init__(self, mgr_list=None, data_list=None, mesh=None, beta=1e-10, order=1, smooth_factor=1, sensitivity_list=None):
        '''
    ...
    def normalize(self, m, m0, idx):
        ...
        mmin = min(np.log(m))
        mmax = max(np.log(m))
        m_new = self.sens[idx]*(np.log(m)-mmin)/(mmax-mmin+self.b)
        m0_new = self.sens[idx]*(np.log(m0)-mmin)/(mmax-mmin+self.b)
        return m_new, m0_new
    ...
    def calcQ_JME(self):
        ...
        for mgr_i, mgr in enumerate(self.mgrs):
            ...
            [m, m0] = self.normalize(m, m0, mgr_i)

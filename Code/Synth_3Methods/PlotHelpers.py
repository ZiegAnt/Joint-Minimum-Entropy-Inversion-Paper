...
def plotJointResultsComparison(mesh, r_est_list, v_est_list, d_est_list,
                               c_ert, c_srt, c_grav, lim_ert, lim_srt, lim_grav, label_list, labels=False,
                               ert_marks=None, srt_marks=None, grav_marks=None, 
                               ert_label=None, srt_label=None, grav_label=None, marks=False,
                               figsize=(10,3.5)):
    ...
    if len(r_est_list)==1:
        print('bla')
        fig, ax, [cax1, cax2, cax3] = plotResultsComparison(mesh, r_est_list[0], v_est_list[0], d_est_list[0], c_ert, c_srt, c_grav,
                                                            lim_ert, lim_srt, lim_grav,
                                                            ert_marks=ert_marks, srt_marks=srt_marks, grav_marks=grav_marks, 
                                                            ert_label=ert_label, srt_label=srt_label, grav_label=grav_label, marks=marks,
                                                            figsize=figsize)
    else:
        ...
        ax[0,2].set_title('GRAV Results', fontsize = 14)
        ...
        for i in range(rows): 
            ...
            pg.show(mesh, d_est_list[i], ax=ax[i,2], 
        ...
        # Add colorbar GRAV
        cax3 = ax[-1,2].inset_axes([0.05, -0.6, 0.9, 0.15])
        createColorBarOnly(ax=cax3, cMin=lim_grav[0], cMax=lim_grav[1], logScale=False,cMap=c_grav,
                          label='Density Contrast (kg/$m^3$)', orientation='horizontal')
        if marks:
            for i, r in enumerate(grav_marks):
                cax3.plot([r]*2, [0,1], 'w')
                cax3.text(r, 1.3, grav_label[i], fontsize=8, horizontalalignment='center', verticalalignment='center')

    return fig, ax, [cax1, cax2, cax3]


def plotResultsComparison(mesh, r_est, v_est, d_est, c_ert, c_srt, c_grav, lim_ert, lim_srt, lim_grav,
                          ert_marks=None, srt_marks=None, grav_marks=None, 
                          ert_label=None, srt_label=None, grav_label=None, marks=None,
                          figsize=(10,3.5)):
    ...
    ax[2].set_title('SRT Results', fontsize = 14)
    ...
    # Add colorbar GRAV
    cax3 = ax[2].inset_axes([0.05, -0.6, 0.9, 0.15])
    createColorBarOnly(ax=cax3, cMin=lim_grav[0], cMax=lim_grav[1], logScale=False,cMap=c_grav,
                          label='Density Contrast (kg/$m^3$)', orientation='horizontal')
    if marks:
        for i, r in enumerate(grav_marks):
            cax3.plot([r]*2, [0,1], 'w')
            cax3.text(r, 1.3, srt_label[i], fontsize=8, horizontalalignment='center', verticalalignment='center')
